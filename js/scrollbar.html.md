...
- **Thumb drag** — pointer events translated to target indices;
  server-authoritative position via HTMX. No client-side optimistic
  thumb movement; the thumb moves only when the server responds with an
  OOB swap carrying fresh data attributes. This matches the purely
  server-driven model used for keyboard navigation in consumer
  libraries, eliminating dual-source-of-truth correction jitter.
- **Track click** — jump to the clicked position (same HTMX round-trip
  shape as drag).
- **Position sync** — after each `htmx:afterSwap` / `htmx:afterSettle`,
  re-read position/totals from the track’s `data-*` attributes and place
  the thumb.
- **Deduplication** — `_lastNavIndex` naturally throttles drag to at
  most one HTMX request per index change (one request per
  ~`track_height / max_position` pixels of drag).

The generated JS is consumer-agnostic. It reads position from the
scrollbar track’s `data-position` attribute and POSTs to a configurable
navigation URL.
...
# --- Pure server-authoritative drag model ---
# Drag fires HTMX; thumb moves via OOB response, not client-side paint.
assert '_onDragMove' in js
# No client-side thumb-position helper should remain (its role is now the OOB response).
assert '_updateThumbPosition' not in js
# No drag-time cursor/height tracking needed in a server-authoritative model.
assert '_lastPointerY' not in js
assert '_dragThumbHeightPx' not in js
# Both HTMX mutation events are wired.
assert "'htmx:afterSwap'" in js
assert "'htmx:afterSettle'" in js
assert '_onHtmxMutation' in js
# Dedup lives in _navToIndex — one HTMX request per distinct target index.
assert 'index === _lastNavIndex' in js
